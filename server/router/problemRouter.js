...
problemRouter.post("/submitStatus", async (req, res) => {
  const submitDetails = req.body;
  const cookies=req.cookies;
  const response = await submitCodeStatus(submitDetails,cookies).catch((err) => {
    return res.status(400).send(err);
  });
  return res.status(200).send(response.data);
});
...
problemRouter.post("/editor", async (req, res) => {
  ...
  const cookies=req.cookies;
  const response = await getQuestionEditorData(editorDetails,cookies).catch((err) => {
    return res.status(400).send(err);
  });
...
problemRouter.post("/testcase", async (req, res) => {
  const problemDetails = req.body;
  const cookies=req.cookies;
  const response = await getTestCaseData(problemDetails,cookies).catch((err) => {
    return res.status(400).send(err);
  });
  return res.status(200).send(response.data);
});
